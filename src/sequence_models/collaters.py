...
class StructureCollater(object):
    ...
    def __init__(self, n_connections=20,
                 n_node_features=10, n_edge_features=11):
        self.n_connections = n_connections
        self.n_node_features = n_node_features
        self.n_edge_features = n_edge_features
        
    def lengths_double(self,attn_masks):
        attn_masks = torch.cat([attn_masks[0][0], attn_masks[0][1]])
        residues = attn_masks == 0
        num_start_end_tokens = 4
        ells = [sum(residues) - num_start_end_tokens]
        max_length = len(attn_masks) - num_start_end_tokens
        return attn_masks, int(max_length), ells
    
    def lengths_single(self,attn_masks):
        attn_masks = torch.cat(attn_masks)
        residues = attn_masks == 0
        num_start_end_tokens = 2
        ells = [sum(residues) - num_start_end_tokens]
        max_length = len(attn_masks) - num_start_end_tokens
        return attn_masks, int(max_length), ells
    
    def __call__(self, batch: List[Any], ) -> Iterable[torch.Tensor]:
        attn_masks, dists, omegas, thetas, phis = tuple(zip(*batch))
        n = len(attn_masks)
        attn_masks, max_ell, ells = self.lengths_single(attn_masks) if isinstance(attn_masks[0], torch.Tensor) else self.lengths_double(attn_masks)
        nodes = torch.zeros(n, max_ell, self.n_node_features, device = 'cuda')
        edges = torch.zeros(n, max_ell, self.n_connections, self.n_edge_features, device = 'cuda')
        connections = torch.zeros(n, max_ell, self.n_connections, dtype=torch.long, device = 'cuda')
        edge_mask = torch.zeros(n, max_ell, self.n_connections, 1, device = 'cuda')
        for i, (ell, dist, omega, theta, phi) in enumerate(zip(ells, dists, omegas, thetas, phis)):
            ...
            gap = max_ell - ell
        return (nodes, edges, connections, edge_mask, gap, max_ell)
